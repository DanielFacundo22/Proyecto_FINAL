...
def historial_arqueo(request):
    ...
    if fecha_apertura:
        ...
        arqueos = ArqueoCaja.objects.prefetch_related('ingresos', 'egresos').order_by('-fecha_hs_apertura')
...
def movimientos_arqueo(request, caja_id):
    ...
    movimientos = Movimiento.objects.filter(caja=caja).annotate(
        ...
        fecha_movimiento=F('fecha'),  # Cambiar a fecha_movimiento para evitar conflictos
    ...
    ingresos = Ingreso.objects.filter(id_caja=caja, tipo='manual').annotate(
        ...
        fecha_movimiento=F('fecha_ingreso'),  # Usar fecha_movimiento
        monto_calculado=F('monto')
    ...
    egresos = Egreso.objects.filter(id_caja=caja).annotate(
        ...
        fecha_movimiento=F('fecha_egreso'),  # Usar fecha_movimiento
        monto_calculado=F('monto')
    ).order_by('-fecha_egreso')

    # Ventas con el número de venta
    ventas = Ventas.objects.filter(id_caja=caja).annotate(
        tipo_literal=Value('Venta', output_field=CharField()),
        fecha_movimiento=F('fecha_hs'),  # Usar fecha_movimiento
        detalle=Func(
            Value('Venta '), F('id_venta'),
            function='CONCAT',
            output_field=CharField()  # Especificar el campo de salida como CharField
        ),
        monto_calculado=F('total_venta')
    ).order_by('-fecha_hs')

    # Unir todos los movimientos y ordenarlos
...
def detalle_arqueo(request, id_caja):
    ...
    # Egresos manuales
    egresos = arqueo.egresos.filter(tipo='manual').annotate(
        tipo_literal=Value('Egreso', output_field=CharField()),
        fecha=F('fecha_egreso')
    )

    # Compras como egresos
    compras = arqueo.compras.all().annotate(
        tipo_literal=Value('Compra', output_field=CharField()),
        fecha=F('fecha_compra'),
        monto=F('total_compra'),
        descripcion=F('descrip_compra')
    )

    # Unificar egresos manuales con las compras para tratarlas como egresos
    todos_egresos = sorted(
        chain(egresos, compras),
        key=attrgetter('fecha'),
        reverse=True
    )

    # Ventas asociadas al arqueo
    ventas = arqueo.ventas.all()

    # Calcular subtotales
    subtotal_ingresos = sum(ingreso.monto for ingreso in ingresos)
    subtotal_egresos = sum(egreso.monto for egreso in todos_egresos)
...
def crear_venta(request):
    ...
    if request.user.username == 'user':
        messages.error(request, "El administrador no hacer ventas.")
        return redirect('inicio')
